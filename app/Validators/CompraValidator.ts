...
export default class CompraValidator {
  ...
  public schema = schema.create({
    ...
    tipoDeCapa: schema.string([rules.alpha({ allow: ['space'] }), rules.maxLength(100)]),

    editora: schema.string([rules.alpha({ allow: ['space'] }), rules.maxLength(100)]),

    bloco: schema.string([rules.alpha({ allow: ['space'] }), rules.maxLength(100)]),
  })
